...
//! This crate contains only deterministic statistics/rules. No network or generative AI.
...
pub fn advise(stats: AnalysisStats) -> Vec<Suggestion> {
    ...
    if stats.white_clip_fraction >= 0.01 || stats.highlight_fraction >= 0.22 {
        let value = -((stats.white_clip_fraction * 900.0) + stats.highlight_fraction * 55.0).clamp(10.0, 45.0);
        out.push(Suggestion {
    ...
    if stats.median_luminance < 0.12 && stats.white_clip_fraction < 0.005 {
        out.push(Suggestion {
            ...
            value: 0.25,
            confidence: 0.64,
            reason: "Median luminance is low and the image has headroom before white clipping.".into(),
        });
    }
    ...
    if stats.estimated_warmth_bias.abs() >= 0.18 {
        out.push(Suggestion {
            ...
            confidence: 0.55,
            reason: "A broad warm/cool cast was detected; this is a relative encoded-image correction, not a physical Kelvin estimate.".into(),
        });
    }
...
mod tests {
    ...
    fn suggests_shadow_lift_only_when_black_clipping_is_not_dominant() {
        let suggestions = advise(AnalysisStats { shadow_fraction: 0.45, black_clip_fraction: 0.004, ..Default::default() });
        assert!(suggestions.iter().any(|item| item.control == "shadows" && item.value > 0.0));

        let clipped = advise(AnalysisStats { shadow_fraction: 0.45, black_clip_fraction: 0.08, ..Default::default() });
        assert!(!clipped.iter().any(|item| item.control == "shadows"));
    }

    #[test]
    fn highlight_rule_is_explainable_and_bounded() {
        let suggestions = advise(AnalysisStats { highlight_fraction: 0.30, white_clip_fraction: 0.025, ..Default::default() });
        let item = suggestions.iter().find(|item| item.control == "highlights").expect("highlight suggestion");
