...
| LibRaw | Production RAW sensor decode, metadata, black/white level, CFA/X-Trans handling, As-Shot WB and mature AHD/X-Trans demosaic boundary | [LibRaw/LibRaw](https://github.com/LibRaw/LibRaw), `0.22.2`, annotated tag object `24fa7e5463cbf8b8615dbd2b16c933a294d52400`, peeled source commit `b93f6e45c194f5df9b02a43b1af9a54b4f41f33f` | **CDDL-1.0 selected for Starroom integration**; upstream is alternatively LGPL-2.1-or-later; bundled DCB/X3F portions retain their BSD notices | No Starroom rewrite or translated port. Unmodified upstream files are vendored and compiled; `crates/starroom-raw/src/libraw_bridge.cpp` is an original narrow adapter | **Yes**, statically compiled by `crates/starroom-raw/build.rs` | **Medium.** CDDL-covered LibRaw files, license, copyright and source are shipped in `vendor/libraw-0.22.2`. Modifications to covered files must remain available under CDDL; Starroom adapter files are separate. Optional DNG SDK/RawSpeed integrations are not enabled. Re-review before binary distribution and on every LibRaw update. |
